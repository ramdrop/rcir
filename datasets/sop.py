...
class Base(data.Dataset):
    def __init__(self, split='train', data_path='', aug=True) -> None:
        ...
        # data_path = 'dbs/SOP/Stanford_Online_Products'
        # debug = False
        # get split indices
        if split == 'train':
            df = pd.read_csv(join(data_path, f"Ebay_train.txt"), sep=" ", header=0)
            self.image_label = df["class_id"].to_numpy()
            self.image_list = np.array([join(data_path, x) for x in df["path"].to_numpy()])
        elif split == 'val':
            df = pd.read_csv(join(data_path, f"Ebay_test.txt"), sep=" ", header=0)
            self.image_label = df["class_id"].to_numpy()
            self.image_list = np.array([join(data_path, x) for x in df["path"].to_numpy()])
        elif split == 'test':
            df = pd.read_csv(join(data_path, f"Ebay_test.txt"), sep=" ", header=0)
...
if __name__ == '__main__':
    whole_val_set = Whole('val', data_path='dbs/SOP/Stanford_Online_Products', aug=False)
    whole_test_set = Whole('test', data_path='dbs/SOP/Stanford_Online_Products', aug=False)
    whole_train_set = Whole('train', data_path='dbs/SOP/Stanford_Online_Products', aug=False)
    print(f'train len: {len(whole_train_set)}')
    print(f'val len: {len(whole_val_set)}')
    print(f'test len: {len(whole_test_set)}')
    whole_val_set.image_list[0]
    whole_train_set.image_list[0]